...
sub component_latest {
    my $self = shift;
    my $limit = shift;
    my $latest = $self->selectall_arrayref("SELECT SQL_CACHE entry_id, title, date, url, description, display_options FROM entries NATURAL JOIN entries_categories WHERE active=1 AND category_id=1 ORDER BY date DESC limit ?",{Slice=>{}}, $limit);
